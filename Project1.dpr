...
begin
  ...
  FDConnection.Params.Values['Database'] := 'contabilidad.db';
...
begin
  ...
  try
    ...
    begin
      Res.Status(400).Send('El nodo <fecha_operacion> no se encontro en el XML');
      Exit;
    end;
...
SetLength(ApuntesTemp, j);
  ...
  try
    ...
    begin
      ...
      // Este segundo "for" es de depuración en la terminal
  ...
  begin
    ...
    try
      ResponseJSON.AddPair('message', 'Operacion registrada correctamente');
...
// Esto no entraba pero quería probar hacer un get que te devuelva la operacion por id
...
begin
  ...
  try
    ...
    if Q.IsEmpty then
    begin
      Res.Status(404).Send(Format('No se encontró la operacion con ID %d', [OperacionID]));
      Exit;
    end;
